...
│       ├── public/               … 静的資産（/assets/** favicon・ロゴ・SNSアイコン）
...
- `apps/web/app/sitemap.ts` … 主要ページのURL等を記載した `sitemap.xml` をビルド時に生成（1パスにつき日英2エントリ + hreflang。**ページ追加時はここに追記する**）。
- `apps/web/app/robots.ts` … `robots.txt` をビルド時に生成。全クローラを許可し、`sitemap.xml` の場所を通知（全パス許可のため**ページ追加時の更新は不要**）。
