...
2. **入力エリア（`.controls`）**
   - 出目の最小値／最大値（0〜100）
   - **よく使う範囲のプリセットボタン（`.preset-buttons`）** … 1〜6 / 1〜10 / 1〜20 / 1〜100 の4種（`PRESETS` 定数）。ワンタップで最小値・最大値を設定（演出中は無効化）
   - サイコロの個数（1〜30）
   - 演出時間（0.5 / 1 / 2 / 3秒。選択は localStorage に保存）
   - 「サイコロを振る」ボタン（演出中は無効化）
...
4. **履歴エリア（`#history-area`）** … 過去の抽選結果一覧 `#history` と「履歴をリセット」ボタン（履歴が空のときは無効化）。各履歴には抽選条件（例: （1〜6 × 2個）、`.history-config`）を付記する。
...
| キー | 型 | 内容 |
| --- | --- | --- |
| `diceHistory` | JSON文字列（`{ results: number[]; min?: number; max?: number }[]`） | 抽選ごとの出目と抽選条件（min / max）を新しい順に格納 |
| `diceDuration` | 文字列（`500` \| `1000` \| `2000` \| `3000`、ミリ秒） | 演出時間の選択状態（`DURATION_OPTIONS` で検証） |

- 復元時は `parseHistory()` で型検証・変換し、不正データはキーごと削除（握りつぶし）。**旧形式（`number[][]`）は「結果のみの履歴」として引き継ぐ**（min / max が無い履歴には抽選条件を表示しない）。
...
| `changeDuration(value)` | 演出時間の変更と localStorage 保存 |
