...
- **当選したことばを一覧から自動で除外するオプション**（順番決め・重複なし抽選向け。チェック状態は保存される）。
- 抽選結果は**履歴として localStorage に保存**（最大50件）され、再訪時に復元される。
- 登録内容・表示方式・抽選時間・当選除外オプションは **localStorage に永続化**され、再訪時に復元される。
...
7. **並び順・抽選時間コントロール（`.sort-controls`）** … 登録が1件以上のとき表示。昇順／降順ソート、抽選時間セレクト（選択は localStorage に保存）。
8. **当選除外オプション（`.spin-options`）** … 登録が1件以上のとき表示。「当選したことばを一覧から除外」チェックボックス（抽選中は変更不可。チェック状態は localStorage に保存）。
9. **ことば一覧（`.word-list`）** … 登録単語をタグ表示。各タグに削除ボタン（×）。
10. **リセットボタン（`.reset-area`）** … 登録が1件以上のとき表示。確認後に全削除。
11. **履歴エリア（`.history-area`）** … 過去の当選ことば一覧（新しい順・最大50件）と「履歴をリセット」ボタン（履歴が空のときは無効化）。
12. **SNSシェア（`ShareButtons`）** … 結果確定後に有効化。ハッシュタグ「単語ルーレット」「ゆるユーリ」。
13. **ツール概要／利用シーン（`.introduction-area`）／使い方ガイド（`.howto-area`）／FAQ（`.faq-area`）**
...
| `wordrouletteHistory` | JSON文字列（string[]） | 当選したことばを新しい順に格納（**上限 `HISTORY_LIMIT = 50` 件**） |
| `wordrouletteRemoveWinner` | 文字列（`true` \| `false`） | 当選除外オプションのチェック状態 |
| `wordrouletteDuration` | 文字列（`0` \| `1` \| `3` \| `5` \| `10`） | 抽選時間の選択状態（`DURATION_OPTIONS` で検証） |

- 復元時は `isStringArray()` 等で型検証し、不正データはキーごと削除（握りつぶし）。
- 保存: 追加・削除・並び替え・表示方式切替・抽選確定（履歴）・オプション変更のたびに保存。
...
| `finalizeWin(word)` | **抽選確定の共通処理**。結果表示（`final`）→ 履歴へ追加 → 当選除外オプションが ON なら当選語を一覧から1件除外（同名重複があっても1件だけ） |
| `spinRoulette()` | 抽選のエントリポイント。表示方式に応じてテキスト演出／`spinWheel()` へ分岐。確定は `finalizeWin()` に集約 |
| `spinWheel(durationSec)` | ホイール式の抽選。当選区画が上部ポインターで止まる回転角を算出 |
| `handleSort(order)` / `resetWords()` / `removeWord(index)` | 並べ替え／全削除（確認あり）／1件削除 |
| `resetHistory()` | 確認後に抽選履歴を全削除 |
| `changeRemoveWinner(checked)` / `changeDuration(value)` | 当選除外オプション／抽選時間の変更と localStorage 保存 |
...
- 単語データ・抽選履歴はブラウザ内のみで完結し、サーバーには送信されない。
