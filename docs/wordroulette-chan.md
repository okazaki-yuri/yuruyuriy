...
4. **抽選結果表示（`.result-box`）** … 演出中は `spin`、確定時は `final` クラスが付く。`role="status"` / `aria-live="polite"` でスクリーンリーダーに通知。
5. **入力モード切替タブ（`.tab-container`）** … 「ことば入力」（1語ずつ）／「まとめて入力」（改行区切りで一括）。
6. **入力エリア（`.input-area`）** … 単語入力 `#wordInput`（最大50文字、Enterで追加可）／一括入力 `#multiInput`（textarea）／「追加」「ルーレット」ボタン（抽選中は「抽選中…」表示で無効化）。
...
- `aria-hidden="true"`（結果は `.result-box` 側で通知するため装飾扱い）。
