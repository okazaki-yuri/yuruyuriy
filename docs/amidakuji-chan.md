...
- 下段のゴール（結果のことば）は自由入力（1行に1つ）。**空欄・不足分は「1, 2, 3…」を自動採番**で補完する（超過分は切り捨て）。
...
10. **ゴール入力（`.goals-area`）** … 結果のことば textarea `#goalsInput`（1行に1つ・行ごとに50文字上限）+ 自動採番のヒント文。
...
| `amidakujiGoals` | JSON文字列（string[]） | ゴール（結果のことば）textarea の行の配列 |
...
| 関数 | 役割 |
| --- | --- |
| `addParticipant()` / `removeParticipant()` | 参加者の追加（重複時は確認ダイアログ）・削除。編集すると `resetRound()` |
| `changeGoalsInput(value)` | ゴールの変更と保存。はしご生成後の編集は `resetRound()` |
