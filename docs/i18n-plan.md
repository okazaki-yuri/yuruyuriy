...
> **ステータス: Phase 1（国際化基盤）完了。Phase 2（英語リソース作成・/en/ 追加）待ち。**
...
### Phase 2: 英語リソース作成と `/en/` ルート追加
1. `en.ts` 辞書を作成（型チェックで漏れ検出）。
2. `app/(en)/en/…` に英語ページを追加（ページ本体は共通コンポーネント + 辞書で構成し、重複を最小化）。
3. 言語スイッチャーを Header / Footer に追加。
4. 英語版 OG 画像の生成を `generate-og.cjs` に追加。
