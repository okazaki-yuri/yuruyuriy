...
> **ステータス: 全フェーズの実装・QA 完了。残りはマージ → デプロイと、デプロイ後の検証（OGP デバッガ・リッチリザルトテスト・Search Console 監視）のみ。**
...
### Phase 4: QA・リリース — **QA 完了（2026-07-23）／リリース待ち**
1. ✅ 全ページ × 両言語をブラウザで表示確認（運営者確認済み）。指摘のあった CSS 疑似要素の文言未対訳を修正し、言語スイッチャーのドロップダウン化・en での LINE/ブログ非表示も対応。
2. ⬜ デプロイ後: OGP デバッガ（X / LINE）、リッチリザルトテスト（FAQ / パンくず）、hreflang の検証。
3. ⬜ デプロイ後: Google Search Console で /en/ のインデックス状況を監視。

### Phase 5: 運用ルールの更新 — **完了（2026-07-23）**
- ✅ `CLAUDE.md`: 「多言語対応（i18n）の運用」セクションを新設（両言語辞書の同時更新・法務ページの追随・ページ追加時の両ラッパー作成・localizePath・言語別フラグ・404 の experimental 注意）。SEO 方針も hreflang / 言語別 lastmod / OG_IMAGE_EN に合わせて更新。
- ✅ `docs/` 各設計書: i18n 構成を反映済み（`docs/i18n.md` 新設、各ページ設計書・`docs/add-new-tool.md` 更新）。
